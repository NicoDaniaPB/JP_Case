...
Join af adfærdsvariabler.
```{r}
model_data <- model_data %>%
  left_join(behavior_agg, by = "pseudo_id")
```

ndledende udvælgelse af klyngevariabler.
```{r}
...
pseudo_id gemmes i en separat vektor inden variablen fjernes fra datasættet, da den ikke må indgå som prædiktor i modellerne, men skal bruges senere til at knytte churn sandsynligheder tilbage til de specifikke kunder. 
```{r}
...
Kontrollerer for høt korrelerede variabler.
```{r}
...
Bestemmelse af optimalt threshold. 
```{r}
...
## Modellering - Chrun kort tid efter kampagnen
